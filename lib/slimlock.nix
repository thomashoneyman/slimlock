{ lib, fetchurl, writeTextFile, nodejs, stdenv, }: rec {
  ...
  getDependencies = lock:
    ...
    in lib.mapAttrs verifyPackage (removeWorkspaces allDependencies);
  ...
  buildPackageLock = { src }:
    let
      ...
      tarballs = builtins.attrValues (builtins.mapAttrs fetchDependencyTarball
        (getDependencies packageLock));
